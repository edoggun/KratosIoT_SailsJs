/**
 * SensorController
 *
 * @description :: Server-side logic for managing Sensors
 * @help        :: See http://sailsjs.org/#!/documentation/concepts/Controllers
 */

var userName = USER_NAME;
var apiName = API_NAME;

module.exports = {
	


  /**
   * `SensorController.sendMessage()`
   */
  sendMessage: function (req, res) {
    return res.json({
      todo: 'sendMessage() is not implemented yet!'
    });
  },


  /**
   * `SensorController.getLatestMessage()`
   */
  getLatestMessage: function (req, res) {
    return res.json({
      todo: 'getLatestMessage() is not implemented yet!'
    });
  },


  /**
   * `SensorController.getMessagesWithInterval()`
   */
  getMessagesWithInterval: function (req, res) {
    return res.json({
      todo: 'getMessagesWithInterval() is not implemented yet!'
    });
  },


  /**
   * `SensorController.getMessageWithCount()`
   */
  getMessageWithCount: function (req, res) {
    return res.json({
      todo: 'getMessageWithCount() is not implemented yet!'
    });
  }
};

